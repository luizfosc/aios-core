# AIOS Ecosystem Index

*Atualizado: 2026-03-18 03:59:53*  
*Regenerar: `node scripts/generate-ecosystem-index.js`*

---

## 🧠 Minds (36)

| # | Mind | Slug | Status | Fidelidade | Especialidade |
|---|------|------|--------|------------|---------------|
| 1 | **Naval Ravikant** | `naval-ravikant` | Complete | Premium | Filosofia, Investimentos |
| 2 | **Tim Ferriss** | `tim-ferriss` | Complete | Premium | Produtividade, Lifestyle Design |
| 3 | **Ítalo Marsili** | `italo-marsili` | Complete | Premium | Psicologia, Filosofia |
| 4 | **Tathi Deandhela** | `tathi-deandhela` | Complete | Premium | Comunicação, Influência |
| 5 | **Luiz Fosc** | `luiz-fosc` | Complete | Premium | Tecnologia, AI, Negócios |
| 6 | **Gui Ávila** | `gui-avila` | Complete | Intermediate | Automação, Negócios |
| 7 | **Thiago Tessman** | `thiago-tessman` | Partial | Intermediate | Conversão, Marketing Digital |
| 8 | **Merlin Mann** | `merlin-mann` | Complete | Intermediate | Produtividade, Criatividade |
| 9 | **Tiago Forte** | `tiago-forte` | Complete | Intermediate | Second Brain, PKM |
| 10 | **Charlie Munger** | `charlie-munger` | Complete | Intermediate | Mental Models, Investimentos |
| 11 | **Paulo Vieira** | `paulo-vieira` | Complete | Premium | Coaching, Inteligência Emocional |
| 12 | **Alex Hormozi** | `alex-hormozi` | Partial | Basic | Ofertas, Escala, Business |
| 13 | **Renner Silva** | `renner-silva` | Sources Only | - | Marketing, Vendas |
| 14 | **Renan Vieira** | `renan-vieira` | Sources Only | - | Funis, Vendas |
| 15 | **Alistair Croll** | `alistair-croll` | Complete | Intermediate | Lean Analytics |
| 16 | **Eliyahu Goldratt** | `eliyahu-goldratt` | Complete | Intermediate | Theory of Constraints |
| 17 | **John Doerr** | `john-doerr` | Complete | Intermediate | OKRs |
| 18 | **Josh Bersin** | `josh-bersin` | Complete | Intermediate | HR, People Analytics |
| 19 | **Kaplan & Norton** | `kaplan-norton` | Complete | Intermediate | Balanced Scorecard |
| 20 | **Martin Fowler** | `martin-fowler` | Complete | Intermediate | Software Architecture |
| 21 | **Neal Ford** | `neal-ford` | Complete | Intermediate | Evolutionary Architecture |
| 22 | **Nicole Forsgren** | `nicole-forsgren` | Complete | Intermediate | DevOps, DORA Metrics |
| 23 | **Simon Wardley** | `simon-wardley` | Complete | Intermediate | Wardley Mapping |
| 24 | **Skelton & Pais** | `skelton-pais` | Complete | Intermediate | Team Topologies |
| 25 | **Annie Duke** | `annie-duke` | Sources Only | - | Decision Making |
| 26 | **Chris Voss** | `chris-voss` | Sources Only | - | Negociação |
| 27 | **Clayton Christensen** | `clayton-christensen` | Sources Only | - | Disruptive Innovation |
| 28 | **Elon Musk** | `elon-musk` | Sources Only | - | Engenharia, Space, AI |
| 29 | **Hamilton Helmer** | `hamilton-helmer` | Sources Only | - | 7 Powers, Strategy |
| 30 | **Keith Cunningham** | `keith-cunningham` | Sources Only | - | Business, Thinking |
| 31 | **Nassim Taleb** | `nassim-taleb` | Sources Only | - | Antifragilidade, Risco |
| 32 | **Robert Cialdini** | `robert-cialdini` | Sources Only | - | Persuasão, Influência |
| 33 | **Shane Parrish** | `shane-parrish` | Sources Only | - | Mental Models |
| 34 | **Steve Jobs** | `steve-jobs` | Sources Only | - | Design, Produto |
| 35 | **Verne Harnish** | `verne-harnish` | Sources Only | - | Scaling Up |
| 36 | **Walt Disney** | `walt-disney` | Sources Only | - | Criatividade, Storytelling |

---

## 🤖 Agentes Core AIOS (24)

| Agente | ID | Especialidade | Autoridade Exclusiva |
|--------|-----|---------------|----------------------|
| **traffic-masters-chief** | `traffic-masters-chief` | | | N/A |
| **tools-orchestrator** | `tools-orchestrator` | | | N/A |
| **story-chief** | `story-chief` | | | N/A |
| **squad** | `squad` | | | N/A |
| **sop-extractor** | `sop-extractor` | | | N/A |
| **pedro-valerio** | `pedro-valerio` | | | N/A |
| **oalanicolas** | `oalanicolas` | | | N/A |
| **legal-chief** | `legal-chief` | | | N/A |
| **design-system** | `design-system` | | | N/A |
| **design-chief** | `design-chief` | | | N/A |
| **db-sage** | `db-sage` | | | N/A |
| **data-chief** | `data-chief` | | | N/A |
| **cyber-chief** | `cyber-chief` | | | N/A |
| **copy-chief** | `copy-chief` | | | N/A |
| **aios-ux** | `aios-ux` | | | N/A |
| **aios-sm** | `aios-sm` | | | N/A |
| **aios-qa** | `aios-qa` | | | N/A |
| **aios-po** | `aios-po` | | | N/A |
| **aios-pm** | `aios-pm` | | | N/A |
| **aios-devops** | `aios-devops` | | | N/A |
| **aios-dev** | `aios-dev` | | | N/A |
| **aios-data-engineer** | `aios-data-engineer` | | | N/A |
| **aios-architect** | `aios-architect` | | | N/A |
| **aios-analyst** | `aios-analyst` | | | N/A |

---

## 🏢 Agentes por Squad (385)

### Squad: advisor-board (14)

- **Walt Disney** — N/A
- **verne-harnish** — N/A
- **Steve Jobs** — N/A
- **Shane Parrish** — N/A
- **Naval Ravikant** — N/A
- **Nassim Taleb** — N/A
- **keith-cunningham** — N/A
- **hamilton-helmer** — N/A
- **elon-musk** — N/A
- **clayton-christensen** — N/A
- **chris-voss** — N/A
- **board-chief** — N/A
- **Annie Duke** — N/A
- **alex-hormozi** — N/A

### Squad: affiliates (17)

- **social-strategist** — N/A
- **social-hooks** — N/A
- **seo-content** — N/A
- **seo-affiliate** — N/A
- **prop-trading-affiliate** — N/A
- **performance-analyst** — N/A
- **niche-ops** — N/A
- **marketplace-ops** — N/A
- **growth-optimizer** — N/A
- **funnel-architect** — N/A
- **email-nurture** — N/A
- **copy-vendas** — N/A
- **content-authority** — N/A
- **authority-builder** — N/A
- **affiliates-chief** — N/A
- **affiliate-strategist** — N/A
- **affiliate-br** — N/A

### Squad: agent-autonomy (6)

- **ToolSmith** — N/A
- **ReasoningEngineer** — N/A
- **EcosystemScout** — N/A
- **AutonomyChief** — N/A
- **AutonomyAuditor** — N/A
- **AgentArchitect** — N/A

### Squad: audio-reels (7)

- **SFX Designer** — N/A
- **Scene Director** — N/A
- **Remotion Assembler** — N/A
- **Reels Optimizer** — N/A
- **Audio Reels Ops** — N/A
- **Image Prompter** — N/A
- **Audio Reels Chief** — N/A

### Squad: branding (7)

- **sagi-haviv** — N/A
- **michael-johnson** — N/A
- **marty-neumeier** — N/A
- **emily-heyward** — N/A
- **brand-chief** — N/A
- **april-dunford** — N/A
- **alexandra-watkins** — N/A

### Squad: business-rules-extraction (8)

- **ronald-ross** — N/A
- **robert-seacord** — N/A
- **michael-feathers** — N/A
- **jan-vanthienen** — N/A
- **james-taylor** — N/A
- **harry-sneed** — N/A
- **bre-chief** — N/A
- **barbara-von-halle** — N/A

### Squad: claude-code-mastery (8)

- **swarm-orchestrator** — N/A
- **skill-craftsman** — N/A
- **roadmap-sentinel** — N/A
- **project-integrator** — N/A
- **mcp-integrator** — N/A
- **hooks-architect** — N/A
- **config-engineer** — N/A
- **claude-mastery-chief** — N/A

### Squad: content-creator (5)

- **video-editor** — N/A
- **scriptwriter** — N/A
- **repurposing** — N/A
- **espiao** — N/A
- **aiox-chief** — N/A

### Squad: content-engine (28)

- **visual-typographer** — N/A
- **visual-technician** — N/A
- **visual-director** — N/A
- **vanessa-lau** — N/A
- **stefan-georgi** — N/A
- **social-strategist** — N/A
- **oraculo-torriani** — N/A
- **nicolas-cole** — N/A
- **justin-welsh** — N/A
- **joanna-wiebe** — N/A
- **icp-rafael** — N/A
- **icp-marcos** — N/A
- **icp-lucas** — N/A
- **icp-camila** — N/A
- **icp-ana** — N/A
- **george-blackman** — N/A
- **gary-vaynerchuk** — N/A
- **gary-halbert** — N/A
- **eugene-schwartz** — N/A
- **devil-advocate** — N/A
- **david-ogilvy** — N/A
- **dan-koe** — N/A
- **dan-kennedy** — N/A
- **copy-chief** — N/A
- **content-engine** — N/A
- **claude-hopkins** — N/A
- **caleb-ralston** — N/A
- **alex-hormozi** — N/A

### Squad: conteudo (10)

- **strategist** — N/A
- **stories-strategist** — N/A
- **reels-creator** — N/A
- **positioning-expert** — N/A
- **content-validator** — N/A
- **content-repurposer** — N/A
- **content-planner** — N/A
- **content-chief** — N/A
- **competitor-analyst** — N/A
- **carousel-creator** — N/A

### Squad: conversao-extrema (6)

- **Traffic Masters Chief** — N/A
- **Tessman Strategist** — N/A
- **Tessman Remarketing** — N/A
- **Tessman Meta Ads** — N/A
- **Tessman Google Ads** — N/A
- **Tessman Copy** — N/A

### Squad: copywriting-squad (16)

- **todd-brown** — N/A
- **stefan-georgi** — N/A
- **jon-benson** — N/A
- **john-carlton** — N/A
- **gary-halbert** — N/A
- **gary-bencivenga** — N/A
- **eugene-schwartz** — N/A
- **david-ogilvy** — N/A
- **dan-koe** — N/A
- **dan-kennedy** — N/A
- **copy-maestro** — N/A
- **clayton-makepeace** — N/A
- **claude-hopkins** — N/A
- **ben-settle** — N/A
- **andre-chaperon** — N/A
- **alex-hormozi** — N/A

### Squad: curator (12)

- **walter-murch** — N/A
- **timestamp-cataloger** — N/A
- **robert-mckee** — N/A
- **mrbeast** — N/A
- **matthew-dicks** — N/A
- **ken-burns** — N/A
- **jonah-berger** — N/A
- **ffmpeg-cutter** — N/A
- **data-curator** — N/A
- **curator-chief** — N/A
- **content-miner-pro** — N/A
- **brendan-kane** — N/A

### Squad: dan-koe (9)

- **koe-sintetizador** — N/A
- **koe-offers** — N/A
- **koe-mindset** — N/A
- **koe-distribuidor** — N/A
- **koe-coletor** — N/A
- **koe-chief** — N/A
- **koe-brand** — N/A
- **koe-arquiteto** — N/A
- **koe-advisor** — N/A

### Squad: design (9)

- **storybook-expert** — N/A
- **page-composer** — N/A
- **nano-banana-generator** — |
- **ds-token-architect** — N/A
- **ds-foundations-lead** — N/A
- **design-chief** — N/A
- **dave-malouf** — N/A
- **dan-mall** — N/A
- **brad-frost** — N/A

### Squad: dopamine-learning (10)

- **schultz-dopamine-analyst** — N/A
- **mcgonigal-resilience-storyteller** — N/A
- **kapp-instructional-integrator** — N/A
- **gee-learning-architect** — N/A
- **fogg-behavior-diagnostician** — N/A
- **eyal-engagement-designer** — N/A
- **dopamine-learning-chief** — N/A
- **deterding-intrinsic-validator** — N/A
- **csikszentmihalyi-flow-calibrator** — N/A
- **chou-core-drives-mapper** — N/A

### Squad: ensinio-mind (1)

- **ensinio-expert** — N/A

### Squad: ensinio-prospector (5)

- **prospector-chief** — N/A
- **prospect-analyst** — N/A
- **outreach-writer** — N/A
- **instagram-researcher** — N/A
- **crm-syncer** — N/A

### Squad: etl-universal-converter (1)

- **etl-chief** — N/A

### Squad: file-research (1)

- **file-hunter** — N/A

### Squad: gui-avila (1)

- **gui-avila** — N/A

### Squad: high-ticket-mastery (1)

- **high-ticket-chief** — N/A

### Squad: high-ticket-sales (1)

- **high-ticket-closer** — N/A

### Squad: hormozi (16)

- **hormozi-workshop** — N/A
- **hormozi-scale** — N/A
- **hormozi-retention** — N/A
- **hormozi-pricing** — N/A
- **hormozi-offers** — N/A
- **hormozi-models** — N/A
- **hormozi-leads** — N/A
- **hormozi-launch** — N/A
- **hormozi-hooks** — N/A
- **hormozi-copy** — N/A
- **hormozi-content** — N/A
- **hormozi-closer** — N/A
- **hormozi-chief** — N/A
- **hormozi-audit** — N/A
- **hormozi-advisor** — N/A
- **hormozi-ads** — N/A

### Squad: icp-cloning (6)

- **synapse-processor** — N/A
- **synapse-architect** — N/A
- **humanizer** — N/A
- **extractor-deep** — N/A
- **clone-validator** — N/A
- **clone-generator** — N/A

### Squad: insight (5)

- **market-scout** — N/A
- **insight-chief** — N/A
- **digital-profiler** — N/A
- **data-storyteller** — N/A
- **conrado-adolpho** — N/A

### Squad: italo-marsili (1)

- **italo-marsili** — N/A

### Squad: jose-amorim (1)

- **jose-amorim** — N/A

### Squad: kaizen (7)

- **TopologyAnalyst** — N/A
- **TechRadar** — N/A
- **PerformanceTracker** — N/A
- **KaizenChief** — N/A
- **CostAnalyst** — N/A
- **CapabilityMapper** — N/A
- **BottleneckHunter** — N/A

### Squad: knowledge-base-builder (8)

- **taxonomy-architect** — N/A
- **quality-validator** — N/A
- **knowledge-linker** — N/A
- **kb-chief** — N/A
- **doc-parser** — N/A
- **content-analyzer** — N/A
- **concept-extractor** — N/A
- **app-architect** — N/A

### Squad: leandro-ladeira (2)

- **leandro-ladeira** — N/A
- **leandro-ladeira-chief** — N/A

### Squad: legacy-rule-extractor (4)

- **decoder** — N/A
- **cartographer** — N/A
- **auditor** — N/A
- **archaeologist** — N/A

### Squad: luiz-fosc (1)

- **luiz-fosc** — N/A

### Squad: magic-bob (3)

- **bob-validator** — N/A
- **bob-orchestrator** — N/A
- **bob-guardian** — N/A

### Squad: marketing-board (12)

- **marketing-production** — N/A
- **marketing-metrics** — N/A
- **marketing-ideation** — N/A
- **marketing-ideation-yt** — N/A
- **marketing-ideation-li** — N/A
- **marketing-ideation-ig** — N/A
- **marketing-distribution** — N/A
- **marketing-designer** — N/A
- **marketing-cmo** — N/A
- **marketing-brand** — N/A
- **carousel-designer** — N/A
- **board-orchestrator** — N/A

### Squad: media-processor (1)

- **media-chief** — N/A

### Squad: mind-cloning (1)

- **mind-cloner** — N/A

### Squad: mind-content-updater (1)

- **mind-content-updater** — N/A

### Squad: mmos-squad (10)

- **system-prompt-architect** — N/A
- **research-specialist** — N/A
- **mind-pm** — N/A
- **mind-mapper** — N/A
- **identity-analyst** — N/A
- **emulator** — N/A
- **debate** — N/A
- **data-importer** — N/A
- **cognitive-analyst** — N/A
- **charlie-synthesis-expert** — N/A

### Squad: money-makers-vtd (6)

- **vtd-chief** — N/A
- **pesquisador-ia** — N/A
- **monetizador** — N/A
- **escalador** — N/A
- **copy-andromeda** — N/A
- **arquiteto-funil** — N/A

### Squad: navigator (1)

- **navigator** — N/A

### Squad: negotiation (9)

- **william-ury** — N/A
- **robert-cialdini** — N/A
- **oren-klaff** — N/A
- **neil-rackham** — N/A
- **negotiation-chief** — N/A
- **jim-camp** — N/A
- **jeb-blount** — N/A
- **chris-voss** — N/A
- **chet-holmes** — N/A

### Squad: palestras-master (5)

- **palestras-quality-gate** — N/A
- **palestras-mind-assembler** — N/A
- **palestras-method-router** — N/A
- **palestras-master-chief** — N/A
- **palestras-diagnostician** — N/A

### Squad: paulo-vieira (1)

- **paulo-vieira** — N/A

### Squad: presenca-digital (9)

- **vanessa-lau** — N/A
- **ross-simmonds** — N/A
- **presenca-digital-chief** — N/A
- **paulo-cuenca** — N/A
- **nicolas-cole** — N/A
- **natanael-oliveira** — N/A
- **justin-welsh** — N/A
- **camilo-coutinho** — N/A
- **brendan-kane** — N/A

### Squad: relationship-therapy-squad (11)

- **terry-real** — N/A
- **sue-johnson** — N/A
- **rosenberg** — N/A
- **lead-therapist** — N/A
- **gottman** — N/A
- **esther-perel** — N/A
- **emily-nagoski** — N/A
- **dr-ramani** — N/A
- **deb-dana** — N/A
- **brene-brown** — N/A
- **amir-levine** — N/A

### Squad: renner-silva (1)

- **renner-silva** — N/A

### Squad: repertoire-mapper (10)

- **Repertoire Chief** — N/A
- **Polanyi** — N/A
- **Nonaka — Knowledge Conversion Architect** — >
- **leonard** — N/A
- **klein** — N/A
- **kelly** — N/A
- **Forte — Knowledge Infrastructure Architect** — >
- **Collins** — N/A
- **Chin — Practitioner Translator** — >
- **Argyris — Gap Detector** — >

### Squad: seo (8)

- **technical-auditor** — N/A
- **site-architect** — N/A
- **seo-chief** — N/A
- **schema-architect** — N/A
- **performance-engineer** — N/A
- **on-page-optimizer** — N/A
- **content-quality-assessor** — N/A
- **ai-visibility-optimizer** — N/A

### Squad: site-performance-audit (8)

- **tim-kadlec** — N/A
- **tammy-everts** — N/A
- **steve-souders** — N/A
- **site-performance-audit-chief** — N/A
- **patrick-meenan** — N/A
- **harry-roberts** — N/A
- **barry-pollard** — N/A
- **addy-osmani** — N/A

### Squad: sop-factory (6)

- **sop-ml-architect** — N/A
- **sop-extractor** — N/A
- **sop-creator** — N/A
- **sop-chief** — N/A
- **sop-auditor** — N/A
- **sop-analyst** — N/A

### Squad: squad-creator (6)

- **squad-diagnostician** — N/A
- **squad-chief** — N/A
- **squad-architect** — N/A
- **sop-extractor** — N/A
- **pedro-valerio** — N/A
- **oalanicolas** — N/A

### Squad: squad-creator-pro (3)

- **squad-chief** — N/A
- **pedro-valerio** — N/A
- **oalanicolas** — N/A

### Squad: storytelling-masters-fosc (12)

- **storytelling-masters-chief** — N/A
- **robert-mckee** — N/A
- **robert-cialdini** — N/A
- **nancy-duarte** — N/A
- **matthew-dicks** — N/A
- **joseph-campbell** — N/A
- **jay-heinrichs** — N/A
- **donald-miller** — N/A
- **dale-carnegie** — N/A
- **chip-dan-heath** — N/A
- **carmine-gallo** — N/A
- **blake-snyder** — N/A

### Squad: tathi-deandhela (5)

- **tathi-voice-dna** — N/A
- **tathi-thinking-dna** — N/A
- **tathi-talk-diagnostician** — N/A
- **tathi-stage-architect** — N/A
- **tathi-chief** — N/A

### Squad: transcript-sculptor (5)

- **transcript-editor** — N/A
- **transcript-analyst** — N/A
- **sculptor-chief** — N/A
- **quality-guardian** — N/A
- **doc-converter** — N/A

### Squad: video-content-distillery (9)

- **youtube-strategist** — N/A
- **Tacit Knowledge Extractor** — N/A
- **Production Ops** — N/A
- **Mental Model Identifier** — N/A
- **Knowledge Architect** — N/A
- **Idea Multiplier** — N/A
- **Ecosystem Designer** — N/A
- **distillery-chief** — N/A
- **Content Atomizer** — N/A

### Squad: zona-genialidade (8)

- **zona-genialidade-chief** — N/A
- **sally-hogshead** — N/A
- **roger-hamilton** — N/A
- **kathy-kolbe** — N/A
- **Gay Hendricks** — Avalia o nivel do termostato interior de sucesso/felicidade
- **don-clifton** — N/A
- **dan-sullivan** — N/A
- **alex-hormozi** — N/A

---

## 📊 Estatísticas

| Categoria | Quantidade |
|-----------|------------|
| **Minds** | 36 |
| **Agentes Core** | 24 |
| **Squads** | 58 |
| **Agentes de Squads** | 385 |
| **TOTAL** | 445 |

---

*Gerado automaticamente em 2026-03-18 03:59:53*
