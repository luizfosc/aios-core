...
Squad especializado em prospectar leads qualificados a partir de exports de grupos de WhatsApp, cruzando com soluções do cliente (5 pilares), ICPs, red flags e gerando mensagens personalizadas de outreach para Google Sheets.
...
### Configuracao
```env
# squads/whatsapp-prospector/.env
...
```
/whatsapp-prospector:workflows:full-pipeline
```
Inputs: `{zip_path}` (path do ZIP exportado) + `{group_name}` (nome do grupo)

### Pipeline Batch (múltiplos grupos)
```
/whatsapp-prospector:workflows:batch-pipeline
```
Inputs: Array de `{zip_path, group_name}` + `{sheet_mode}` (single_tab | new_tab_per_group | append)

### Tasks Individuais
```
/whatsapp-prospector:tasks:parse-chat-export        # P1: Parsear ZIP WhatsApp
/whatsapp-prospector:tasks:validate-parsed-data     # P1b: Validar dados parseados
/whatsapp-prospector:tasks:load-ensinio-kb          # P2: Carregar KB Ensinio (5 pilares)
/whatsapp-prospector:tasks:resolve-phone-numbers    # P2b: Resolver telefones (interativo)
/whatsapp-prospector:tasks:analyze-prospects         # P3: Analisar, ICP match, red flags, scorer
/whatsapp-prospector:tasks:write-outreach            # P4: Gerar mensagens personalizadas
/whatsapp-prospector:tasks:validate-outreach-batch   # P4b: Validar batch de mensagens
/whatsapp-prospector:tasks:sync-to-ghl               # P5: Sync GHL (Contact+Deal+Msg) [NEW v4.0]
/whatsapp-prospector:tasks:populate-sheet            # P6: Popular Google Sheets
/whatsapp-prospector:tasks:handle-parse-errors       # Recovery: Tratar erros de parse
```
...
*WhatsApp Prospector v4.0.0 - AIOS Squad*
