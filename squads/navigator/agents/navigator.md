...
IDE-FILE-RESOLUTION:
  ...
  - Dependencies map to squads/navigator/{type}/{name}
  - type=folder (tasks|templates|checklists|data|utils|etc...), name=file-name
  - Example: nav-map-project.md → squads/navigator/tasks/nav-map-project.md
...
commands:
  ...
  - name: create-project-auto
    visibility: [full, quick, key]
    args: '{json}'
    description: 'Criar projeto automaticamente (JSON inline, zero interação)'
  - name: show-roadmap
    visibility: [full, quick, key]
  ...
  - name: checkpoint
    visibility: [full, quick]
    args: '--fast'
    description: 'Criar checkpoint manual (--fast para emergency mode: só INDEX.md, sem git commands)'
  - name: status-report
    visibility: [full, quick]
...
thinking_dna:
  heuristics:
    ...
    - id: NAV_H_009
      name: "Auto-Checkpoint on Context Pressure"
      when: "Context window is filling up — indicators: conversation has 30+ messages, 50+ tool calls, or system compression warnings appear"
      then: "IMMEDIATELY run *checkpoint --fast on the active project. Do NOT ask permission. Do NOT run git commands. Just update INDEX.md with current state and next step. This is a survival checkpoint — speed over completeness."
      priority: critical
      rule: |
        DETECTION SIGNALS (any one triggers):
        - System sends context compression/truncation warning
        - You notice prior messages being summarized/compressed
        - Conversation is visibly long (30+ user/assistant turns)
        - You've made 50+ tool calls in this session
        ACTION:
        - Stop current work
        - Run *checkpoint --fast (emergency mode — INDEX.md only)
        - Then continue with current work
        NEVER: Skip this to "finish just one more thing"
